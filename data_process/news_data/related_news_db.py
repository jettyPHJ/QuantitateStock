...
class RelatedNewsDBManager:
    ...
    def __init__(self, sector_name: str, core_stock_tickers: List[str], db_dir: str = "related_news_db"):
        ...
        # 数据库文件名直接使用板块中文名
        self.db_file = os.path.join(self.db_dir, f"{self._format_db_name(self.sector_name)}.db")
        ...
        for year in range(start_year, now.year + 1):
            ...
            for month in range(1, end_month + 1):
                ...
                try:
                    ...
                    print(f"[ERROR] 在处理 {self.sector_name} {year}-{month:02d} 时发生错误: {e}")
                    self.conn.rollback()
...
def create_related_news_db(entry_key: str):
    ...
    for _, item in sub_items.items():
        ...
        RelatedNewsDBManager(sector_name=item.name, core_stock_tickers=core_tickers)
        print(f"===== [INFO] 板块 {item.name} 新闻更新完成 =====\n")


# --------------------- 测试入口 ---------------------
if __name__ == "__main__":
    core_tickers = get_stock_codes(Block.get("半导体产品").code)
    RelatedNewsDBManager("半导体产品", core_tickers)
